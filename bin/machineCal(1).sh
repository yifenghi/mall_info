...
htext $output/p* > $work_dir/data/data_tmp

cat $work_dir/data/data_tmp | \
awk -F "	" '{
  if($2 ~ /MACHINE/) {
    print $1
  }
}' > $work_dir/data/machine_list.new

cat $work_dir/data/machine_list.new $work_dir/data/machine_list |\
sort -u > $work_dir/data/machine_list.merge
mv $work_dir/data/machine_list.merge $work_dir/data/machine_list
...
}' > $work_dir/data/employee_list.new

cat $work_dir/data/employee_list.new $work_dir/data/employee_list |\
sort -u > $work_dir/data/employee_list.merge
mv $work_dir/data/employee_list.merge $work_dir/data/employee_list

rm $work_dir/data/data_tmp $work_dir/data/machine_list.new $work_dir/data/employee_list.new
exit 0;
