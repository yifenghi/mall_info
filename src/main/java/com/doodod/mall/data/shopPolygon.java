...
public class shopPolygon {
    ...
    public static void sendPostData(String POST_URL)
            throws Exception {
    ...
    private static void getShopPolygon(String type, String path, int floorId, String res) throws Exception {
				...
				if (geometryType.equals("Polygon")) {
					...
					for (int a = 0; a < coorArr.length(); a++) {
						...
						for (int b = 0; b < coor.length(); b++) {
							polygonBuffer.append(coor.getJSONArray(b).getString(0));
							polygonBuffer.append(Common.CTRL_C);
							polygonBuffer.append(coor.getJSONArray(b).getString(1));
    ...
    public static List<Integer> getFloorIdList(int mallId, String hostName) throws Exception {
    	List<Integer> floorList = new ArrayList<Integer>();
    	
    	//http://apiv2.palmap.cn/mall/67/floors?appkey=123&udid=456
		String res = sendGetData(hostName + "/mall/" 
				+ String.valueOf(mallId) + "/floors?appkey=123&udid=456");
		
		JSONArray floorArray = new JSONArray(res);
		//index 0 is 平面图, skip
		for (int i = 1; i < floorArray.length(); i++) {
			JSONObject floorInfo = floorArray.getJSONObject(i);
			floorList.add(Integer.parseInt(
					floorInfo.get("id").toString()));
		}
...
//		String path = "/Users/paul/Documents/code_dir/doodod/mall_info/data/";
