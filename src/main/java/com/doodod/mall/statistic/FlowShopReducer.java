...
public class FlowShopReducer extends 
	...
	private Map<Long, Integer> SHOP_AVG_MAP = new HashMap<Long, Integer>();
	private Map<Long, Integer> ZONE_AVG_MAP = new HashMap<Long, Integer>();
	private Map<Long, List<Integer>> SHOP_DIS_MAP = new HashMap<Long, List<Integer>>();
	private Map<Long, List<Integer>> ZONE_DIS_MAP = new HashMap<Long, List<Integer>>();
	...
	public void reduce(Text key, Iterable<BytesWritable> values, Context context)
		...
		if (key.toString().startsWith(Common.FLOW_TAG_DAY)) {
			...
			SHOP_AVG_MAP.put(shopId, shopAvg);
			SHOP_DIS_MAP.put(shopId, visitShopList);
			...
			ZONE_AVG_MAP.put(shopId, zoneAvg);
			ZONE_DIS_MAP.put(shopId, visitZoneList);
	...
	public void cleanup(Context context)
		...
		while (iter.hasNext()) {
			...
			String tag = arr[0];
			int shopCat = Integer.parseInt(arr[2]);
		...
		Iterator<Long> visitIter = SHOP_DIS_MAP.keySet().iterator();
		while (visitIter.hasNext()) {			
			long shopId = visitIter.next();			
	        ...
	        List<Integer> visitShopList = SHOP_DIS_MAP.get(shopId);
			...
			int shopAvg = SHOP_AVG_MAP.get(shopId);
			...
			List<Integer> visitZoneList = ZONE_DIS_MAP.get(shopId);
			...
			int zoneAvg = ZONE_AVG_MAP.get(shopId);
