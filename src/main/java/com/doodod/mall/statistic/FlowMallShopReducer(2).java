...
public class FlowMallShopReducer extends Reducer<Text, Text, Text, Text> {
	...
	private static double AVG_SHOP_VISIT;
	private static double AVG_ZONE_VISIT;
	...
	public void reduce(Text key, Iterable<Text> values, Context context)
		...
		AVG_SHOP_VISIT = sumShop / (1.0 * custNum);
		AVG_ZONE_VISIT = sumzone / (1.0 * custNum);
		
		DecimalFormat df = new DecimalFormat(Common.NUM_FORNAT);
		context.write(key, new Text(df.format(AVG_SHOP_VISIT) + Common.CTRL_A
				+ df.format(AVG_ZONE_VISIT)));
	}
