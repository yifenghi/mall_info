...
public class DwellFloorReducer extends 
	...
	public void reduce(Text key, Iterable<Text> values, Context context)
		...
		floorId = Long.parseLong(key.toString().split(Common.CTRL_A, -1)[1]);
		...
		DWELL_MAP.put(floorId, dwellList);
		AVG_DWELL_MAP.put(floorId, timeAvg);
	}
	...
	public void cleanup(Context context)
		...
		while (iter.hasNext()) {
			...
			List<Integer> dwellList = DWELL_MAP.get(floorId);
			int timeAvg = AVG_DWELL_MAP.get(floorId);
			...
			document.append(Common.MONGO_OPTION_SET, 
					new BasicDBObject(visitDwell, timeAvg));
