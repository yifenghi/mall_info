...
public class VisitCustomerReducer extends 
	...
	public void reduce(LongWritable key, Iterable<Text> values, Context context)
		...
		for (Text text : values) {
			String arr[] = text.toString().split(Common.CTRL_A, -1);
	...
	public void cleanup(Context context)
		...
		KEY_MALLID = context.getConfiguration().get(Common.MONGO_COLLECTION_COUNT_ID);
		KEY_CREATETIME = context.getConfiguration().get(Common.MONGO_COLLECTION_COUNT_TIME);
		...
		DBCollection countCollection = 
				mongoDb.getCollection(countCollectionName);
		
		Iterator<Long> iter = MALL_COUNT_MAP.keySet().iterator();
		while (iter.hasNext()) {
			long mallId = iter.next();
			BasicDBObject doc = MALL_COUNT_MAP.get(mallId);
			
			BasicDBObject query = new BasicDBObject();
			query.put(KEY_MALLID, mallId);
